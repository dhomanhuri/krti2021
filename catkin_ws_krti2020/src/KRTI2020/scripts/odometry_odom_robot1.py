...
class Server:
	...
	def update_position(self):
		self.current_time = rospy.Time.now()
		rotmat = sp.Matrix([[sp.cos(-0.5*pi), -sp.sin(-0.5*pi), 0.], [sp.sin(-0.5*pi), sp.cos(-0.5*pi), 0.], [0., 0., 1.]])
		...
		#compute x dot
		x_dot = rotmat*J*self.w_enc
		self.Jinv = J.inv()
		#scalling the x_dot
		x_dot[0,0] = x_dot[0,0] * 6. / 0.49# * 6. / 9.3
		x_dot[1,0] = x_dot[1,0] * 9. / 0.68# * 9. / 13.74
		x_dot[2,0] = x_dot[2,0] * 2.0 * pi# * 6.28 / 20.40
		#twist value
		vx = x_dot[0,0] 
		vy = x_dot[1,0] 
		vth = -x_dot[2,0]
		#update position
		self.position[0,0] = self.position[0,0] + vx * self.ts #x
		self.position[1,0] = self.position[1,0] + vy * self.ts #y
		self.position[2,0] = self.position[2,0] + vth * self.ts #th
